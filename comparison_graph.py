...
data = np.loadtxt("data/rsq_BETA100.dat", skiprows=1)
...
data = np.loadtxt("data/npoly_BETA100.dat", skiprows=1)
pop = data[:,1]

fit_param = np.polyfit(np.log(N[2:50]-1), np.log(rsq_PERM[2:50]), 1)
print fit_param

rsq_teo = np.exp([fit_param[1] + 2*0.5876*log(y-1) for y in N[2:]])
